    ...
    def makeCard(self):
        ...
        for a in range(0, self._count):
            ...
            while True:
                ...
                if first == final:
                    break
            self._card_list.append(card_value_list)
        print(time.time() - start_time)
        #self.print_card(devide)

    def print_card(self, devide):
        #지수 변수
        p = 0 
        #결과값
        res = 0
        #반복문 돌면서 res 계산
        for c in range(0,self._count):
            get_bi = self.my_card(c, devide)
            pp = pow(2,p)
            res = res + get_bi * pp 
            p = p+1
        if res != 0:
        ...
        while True:
            ...
            for i in range(0,loop):
                for j in range(0,cell_loop):
                    print('%7d'% self._card_list[index][arr_index], end ='')
